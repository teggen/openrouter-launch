...
| Pushed | Yes, both branches, and they are **level** at `aeefb91` — the `v0.3.0` commit — except for this handoff update itself, which lands on `develop` and un-levels them again, exactly as the note below describes. Historical: after `v0.2.0`, `origin/main` was at `70970d8` and `origin/develop` was **one commit ahead**, and that one commit is this handoff update itself — the fast-forward left the branches level and then writing this row un-levelled them, which is the whole reason this row keeps going stale. No unreleased *code* is outstanding. Check `git status -sb` and `git log --oneline main..develop` rather than trusting this row; it has been wrong before, and it went stale twice within an hour of being written. |
